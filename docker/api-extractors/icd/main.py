...
# NIH Clinical Tables Search API (ICD-10 CM)
API_URL = "https://clinicaltables.nlm.nih.gov/api/icd10cm/v3/search"

def fetch_icd_data(term="diabetes", limit=50):
    print(f"Fetching ICD-10 data for term: {term}...")
    params = {
        'terms': term,
        'maxList': limit
    }
    ...
    try:
        ...
        # API returns [count, codes, details, names]
        data = response.json()
        ...
        print(f"Error fetching ICD data: {e}", file=sys.stderr)
        return []
...
def upload_to_adls(data, storage_account_name, storage_account_key, container_name):
    try:
        ...
        directory_name = "icd_data"
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        file_name = f"icd_extract_{timestamp}.json"
